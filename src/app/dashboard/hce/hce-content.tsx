...
import { FileText, Telescope, Search, Clock, ChevronRight, User as UserIcon } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';
import { formatDistanceToNow } from 'date-fns';
import { es } from 'date-fns/locale';

interface HceContentProps {
  recentPatients: any[];
}

export function HceContent({ recentPatients }: HceContentProps) {
          ...
          <ScrollArea className="flex-1 -mx-2 px-2">
            <div className="space-y-2 pb-4">
              {recentPatients.length === 0 ? (
                <p className="text-xs text-muted-foreground px-2 italic">No hay registros recientes.</p>
              ) : (
                recentPatients.map((patient) => (
                  <div 
                    key={patient.id} 
                    onClick={() => setSelectedPersona(patient)}
                    className="group flex items-center justify-between p-3 rounded-xl bg-card/50 border border-border/50 hover:bg-card hover:shadow-sm hover:border-blue-100 transition-all cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-foreground/90 group-hover:text-blue-700 transition-colors">
                            {patient.nombreCompleto}
                        </p>
                        <p className="text-xs text-muted-foreground/80 flex items-center gap-1">
                          <Clock className="h-3 w-3" /> {formatDistanceToNow(new Date(patient.createdAt), { addSuffix: true, locale: es })}
                        </p>
                      </div>
                    </div>
            ...
            <div className="px-8 py-6 border-b border-border/50 bg-card z-10">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2 bg-muted/50 px-4 py-2 rounded-lg border border-border/50">
                  <UserIcon className="h-5 w-5 text-muted-foreground/80" />
