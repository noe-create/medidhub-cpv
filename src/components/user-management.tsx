...
export function UserManagement({ roles }: UserManagementProps) {
  ...
  const columns: ColumnDef<User>[] = [
    ...
    { accessorKey: 'name', header: 'Nombre Completo', cell: ({ row }) => row.original.name || <span className="text-muted-foreground">N/A</span> },
    { accessorKey: 'role.name', header: 'Rol', cell: ({ row }) => <Badge variant="outline" className="capitalize">{row.original.role?.name}</Badge> },
    {
      ...
      cell: ({ row }) => {
        ...
        // A non-superuser cannot edit or delete a superuser.
